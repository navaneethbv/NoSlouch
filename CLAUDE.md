...
- **Reminder engine.** `PostureNotifier.nudgeReminder(kind:)` handles a unified `ReminderKind` set (`breakTime`, `eyeRest`, `hydration`, `movement`). The ViewModel fires them from `handle()` on accumulated monitored time (`monitoredSeconds`, which keeps advancing while the analyzer is uncalibrated) with per-kind markers, a global minimum gap (no stacking), and deferral during mute-in-meetings, away, or quiet hours (`quietHoursEnabled`, `quietStartMinutes`/`quietEndMinutes`). Inter-reading gaps over 300 s are treated as delivery stalls and book no time (`maxReadingGapSeconds`). Break-reminder behavior is unchanged; the fake's `breakNudgeCount` now derives from the `.breakTime` count.
...
- **AirPodsBatteryMonitor** reads AirPods/Beats battery by running `/usr/sbin/system_profiler -json SPBluetoothDataType` (locale-independent keys; only devices reporting left/right bud levels count, AirPods/Beats preferred), polling every 300 s. The subprocess is drained before `waitUntilExit()` with a kill-after-timeout (pipe-deadlock fix), a generation counter drops in-flight results after `stop()`, and tests inject `fetchRawOutput` instead of spawning processes. It drives the menu battery widget and a fire-once low-battery warning (`lowBatteryWarningEnabled`, buds only — the case is ignored). Shelling out means battery is unavailable under App Sandbox (documented caveat in the file).
- **Pure insight types** (no imports, unit-test priority alongside `SlouchEngine`): `DetectionPreset` (Gentle/Standard/Strict sensitivity), `StreakCalculator` (daily-goal streaks; an unmet *today* is pending, not a miss), `PostureGrade` + `Achievements`, `WeeklyDigest`, `TrendAggregator` (day/week/month rollups for the history chart). `PostureHistoryStore` retains hourly buckets (sessions are split across the hours/days they span, pro rata), backs up undecodable blobs to `<key>.corrupt`, and offers `exportCSV()`.
- **Weekly digest delivery.** `maybeSendWeeklyDigest()` fires a `.passive` notification at most once per 7 days (`settings.lastWeeklyDigestDate`), checked after each authorization refresh and on session finalize; the first activation anchors the date without firing, and the date is only consumed when notifications are enabled.
- **Notification banner actions (B3).** The posture nudge carries a `UNNotificationCategory` with "Snooze 15 min" and "Recalibrate"; `PostureNotifying.onAction` routes taps (on main) to `snoozeNudges(for:)`/`calibrateAveraged()`. The category is registered in `PostureNotifier.init`, before any nudge can post.
- `PostureViewModel.settings` is `@Published private(set)` — all mutation goes through the tier-aware `update…` methods; `SlouchEngine.update` ignores non-finite pitch/roll samples.
...
- `AudioOutputMonitor` dispatches its CoreAudio listener on `DispatchQueue.main`. All reads and writes of `isHeadphoneOutput` (formerly `airPodsActive` — renamed because it is true for any headphone-ish/Bluetooth output, BUG-2) happen on the main thread; the ViewModel reads it from the main thread only.
- **Automation (E2).** The `noslouch://` URL scheme (registered in `Resources/Info.plist`) drives `start`/`stop`/`calibrate`/`resume`/`snooze?minutes=N` via the pure `URLCommand.parse` and `PostureViewModel.handle(_:)`; delivery hooks into `onOpenURL` on the menu-bar label. App Intents/Siri would require the Xcode-project migration (same bucket as WidgetKit).
- **Release path (F4).** `make dmg` builds a distributable image; `make notarize` (needs `SIGN_IDENTITY` + a notarytool keychain profile) submits and staples it. The channel decision (Developer ID DMG, not App Store — keeps the battery widget despite NB-6) and full runbook live in `docs/dev/RELEASE.md`.
...
- **calibratedBaselinePitch / calibratedBaselineRoll lifecycle**: Both persisted in `AppSettings` on calibration (`calibrate()`) and auto-loaded on launch (roll matters for tilt detection — restoring with roll 0 would false-alarm users whose natural sensor roll exceeds the tilt threshold). Any change to the analyzer-affecting setting tier calls `saveSettingsAndResetAnalyzer()`, which clears both (`nil`) to avoid stale baselines.
