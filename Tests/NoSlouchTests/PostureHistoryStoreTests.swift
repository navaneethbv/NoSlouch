...
final class PostureHistoryStoreTests: XCTestCase {
    ...
    func testHistoryAggregatesSessionsByDay() throws {
        ...
        store.add(PostureSession(startedAt: day.addingTimeInterval(3_600), endedAt: day.addingTimeInterval(3_660), badSeconds: 20))

        XCTAssertEqual(store.stats, [
    ...
    func testHistoryIgnoresShortSessions() throws {
        let store = PostureHistoryStore(defaults: defaults)
        let start = try XCTUnwrap(Calendar(identifier: .gregorian).date(from: DateComponents(year: 2026, month: 6, day: 29)))

        store.add(PostureSession(startedAt: start, endedAt: start.addingTimeInterval(4.99), badSeconds: 4))
    ...
    func testHistoryEvictsEntriesOlderThanNinetyDays() throws {
        ...
        XCTAssertEqual(store.stats.first?.day, try XCTUnwrap(calendar.date(byAdding: .day, value: 1, to: firstDay)))
        XCTAssertEqual(store.stats.last?.day, try XCTUnwrap(calendar.date(byAdding: .day, value: 90, to: firstDay)))
    }
